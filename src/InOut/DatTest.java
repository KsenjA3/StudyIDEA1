...
// не получается разобраться с кодировкой в файле кракозяблы
        try (var dataOut = new DataOutputStream(new  //  FileOutputStream ("testdat.txt")))
                PrintStream("testdat.txt", "Cp1251"))) {
        ...
        try (var dataIn = new DataInputStream(new FileInputStream("testdat.txt"))) {
            System.out.println("Прочитано: " + dataIn.readInt());

            System.out.println("Прочитано: " + dataIn.readDouble());

            System.out.println("Прочитано: " + dataIn.readBoolean());

            System.out.println("Прочитано: " + dataIn.readUTF());
        ...
        try (var dataIn = new DataInputStream(new FileInputStream("pass.txt"))) {
            System.out.println("Прочитано: " + dataIn.readUTF());
            System.out.println("Прочитано: " + dataIn.readUTF());
            System.out.println("Прочитано: " + dataIn.readUTF());
        } catch (IOException e) {
